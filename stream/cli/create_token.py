...
@click.option("--expiration", type=int, help="Token expiration time in seconds (e.g., 3600 for 1 hour).")
@click.option("--channel-cids", type=str, help="Channel CIDs to generate the token for.")
...
def create_token(api_key, api_secret, user_id, expiration, channel_cids,role,call_cids):
    ...
    token = client.create_token(user_id, expiration=expires_in,channel_cids=ccids, role=r,call_cids=cids)
