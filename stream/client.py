...
class BaseClient:
    ...
    async def post(self, path,data=None,  json=None,params=None, files=None, headers=None):
        config = await self.config( headers)
        return await self.client.post(self.format_url(path), json=json,data=data, files =files,params=params, **config)
    
    async def put(self, path, json=None,data=None, content=None,files=None, params=None, headers=None):
        config = await self.config(path, headers)
        return await self.client.put(self.format_url(path), json=json,data=data,content=content,files=files,params=params, **config)
