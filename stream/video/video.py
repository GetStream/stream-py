...
class Video(BaseClient):
    def __init__(self, stream:Stream):
        ...
        super().__init__(self.stream.admin_token, "https://video.stream-io-api.com/video")
    
    async def edges(self)-> GetEdgesResponse:
        response = await self.stream.get("/edges")
        return GetEdgesResponse(**response.json())
    
    async def create_call_type(self,data: CreateCallTypeRequest)-> CreateCallTypeResponse:
    ...
    async def update_call_type(self, name: str, data: UpdateCallTypeRequest)-> UpdateCallTypeResponse:
    ...
    async def get_or_create_call(self, calltype: str, callid: str, data: GetOrCreateCallRequest)-> GetOrCreateCallResponse:
        response = await self.stream.video.post(f"/calls/{calltype}/{callid}", data=data)
