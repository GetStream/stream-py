...
class InvalidWebhookError(Exception):
    """Single error class for all webhook handling failures.

    Catch this for any webhook problem; filter on the message text or the
...
@dataclass
class UnknownEvent:
    """Returned by parse_event when the type discriminator is well-formed but unknown.

    Stable forward-compat surface: backend may add new event types before this SDK
    is regenerated. Switch on the returned object's class and include UnknownEvent
    as a fallback case.

    Fields:
        type: the unrecognized discriminator value
        created_at: parsed timestamp from the envelope, or None if the field was missing or unparseable
        raw: the full parsed JSON dict for inspection
    """
...
def parse_webhook_event(raw_event: Union[bytes, str, Dict[str, Any]]) -> Any:
    ...
    try:
        ...
        raise ValueError(f"Failed to deserialize webhook event: {e}") from e
...
def gunzip_payload(body: bytes) -> bytes:
    """Decompress body if gzip-prefixed, else pass through.
    ...
    try:
        ...
        raise InvalidWebhookError(f"{InvalidWebhookError.GZIP_FAILED}: {e}") from e
...
def verify_signature(body: bytes, signature: str, secret: str) -> bool:
    """HMAC-SHA256 over body. Expects already-decompressed bytes.

    Canonical name; aliases the existing verify_webhook_signature.
    """
    return verify_webhook_signature(body, signature, secret)


def parse_event(payload: bytes) -> Any:
    ...
    event_class = _get_event_class(event_type)
    if event_class is None:
        return _build_unknown_event(event_type, data)
...
def _build_unknown_event(event_type: str, data: Dict[str, Any]) -> UnknownEvent:
    ...
    if isinstance(created_raw, str):
        try:
            ...
            created = None
    return UnknownEvent(type=event_type, created_at=created, raw=data)


def verify_and_parse_webhook(body: bytes, signature: str, secret: str) -> Any:
    """HTTP composite: gunzip (if gzip-prefixed) -> verify signature -> parse.
    ...
    *uncompressed* JSON body, hex-encoded. Magic-byte detection means callers
    ...
    Raises:
        InvalidWebhookError: for every failure mode. Inspect the message text or
            match against the failure-mode constants on the class
            (SIGNATURE_MISMATCH, INVALID_BASE64, GZIP_FAILED, INVALID_JSON) to
            differentiate.
    """
    payload = gunzip_payload(body)
    if not verify_signature(payload, signature, secret):
        raise InvalidWebhookError(InvalidWebhookError.SIGNATURE_MISMATCH)
...
def parse_sqs(message_body: str) -> Any:
    ...
    therefore performs no signature verification. If the backend ever adds one,
    a signed variant will be added rather than retrofitting this signature.
    """
    payload = decode_sqs_payload(message_body)
    return parse_event(payload)
