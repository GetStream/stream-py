...
# repo's generate.sh copies it into tests/assets/webhooks/ at codegen time
# (per stream-py AGENTS.md §8: test file assets live under tests/assets/).
# ---------------------------------------------------------------------------

FIXTURE_ROOT = Path(__file__).resolve().parents[2] / "tests" / "assets" / "webhooks"
...
class TestWebhookConformanceHappy:
    ...
    def test_composite_http_identity(self, fixture_dir):
        body = _read_bytes(fixture_dir / "body.json")
        sig = _read_str(fixture_dir / "signature.txt")
        event = verify_and_parse_webhook(body, sig, CANONICAL_TEST_SECRET)
        assert event.type == fixture_dir.name

    def test_composite_http_gzip(self, fixture_dir):
        body_gz = _read_bytes(fixture_dir / "body.gz")
        sig = _read_str(fixture_dir / "signature.txt")
        event = verify_and_parse_webhook(body_gz, sig, CANONICAL_TEST_SECRET)
        assert event.type == fixture_dir.name
