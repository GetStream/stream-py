...
# chat/'s `chat-manager openapi generate-webhook-fixtures` subcommand; the
# repo's generate.sh copies it into tests/fixtures/webhooks/ at codegen time.
# ---------------------------------------------------------------------------

FIXTURE_ROOT = Path(__file__).resolve().parents[2] / "tests" / "fixtures" / "webhooks"
...
@pytest.mark.parametrize("fixture_dir", _fixture_event_dirs(), ids=lambda d: d.name)
class TestWebhookConformanceHappy:
    ...
    def test_verify_signature(self, fixture_dir):
        body = _read_bytes(fixture_dir / "body.json")
        sig = _read_str(fixture_dir / "signature.txt")
        assert verify_signature(body, sig, CANONICAL_TEST_SECRET)

    def test_composite_http_identity(self, fixture_dir):
        body = _read_bytes(fixture_dir / "body.json")
        sig = _read_str(fixture_dir / "signature.txt")
        verify_and_parse_webhook(body, sig, CANONICAL_TEST_SECRET)  # must not raise

    def test_composite_http_gzip(self, fixture_dir):
        body_gz = _read_bytes(fixture_dir / "body.gz")
        sig = _read_str(fixture_dir / "signature.txt")
        verify_and_parse_webhook(body_gz, sig, CANONICAL_TEST_SECRET)
...
class TestWebhookConformanceNegative:
    def _neg(self, name):
        return FIXTURE_ROOT / "_invalid" / name
    ...
    def test_bad_compression(self):
        ...
        with pytest.raises(InvalidWebhookError, match="gzip decompression failed"):
            verify_and_parse_webhook(body, sig, CANONICAL_TEST_SECRET)
