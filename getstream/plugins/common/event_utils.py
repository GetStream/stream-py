...
from .events import (
    BaseEvent, EventType, ConnectionState,
    STTTranscriptEvent, STTPartialTranscriptEvent, STTErrorEvent,
    TTSAudioEvent, TTSSynthesisStartEvent, TTSSynthesisCompleteEvent,
    VADAudioEvent, VADPartialEvent,
    STSConnectedEvent, STSDisconnectedEvent,
    create_event
)
...
class EventSerializer:
    """Serialize and deserialize events for storage or transmission."""
    ...
    def serialize_event(event: BaseEvent) -> Dict[str, Any]:
        ...
        if hasattr(event, 'audio_data') and event.audio_data:
            ...
            data['audio_data'] = {
                'size_bytes': len(event.audio_data),
                'type': 'bytes'
            }
    ...
    def serialize_events(events: List[BaseEvent]) -> str:
        ...
        serialized_events = [
            EventSerializer.serialize_event(event) for event in events
        ]
    ...
    def deserialize_event(data: Dict[str, Any]) -> BaseEvent:
        ...
        if 'timestamp' in constructor_data and isinstance(constructor_data['timestamp'], str):
            constructor_data['timestamp'] = datetime.fromisoformat(constructor_data['timestamp'])
        ...
        if 'error' in constructor_data and isinstance(constructor_data['error'], dict):
            ...
            constructor_data['error'] = Exception(error_info.get('message', 'Unknown error'))
        
        # Remove audio data placeholder
        if 'audio_data' in constructor_data and isinstance(constructor_data['audio_data'], dict):
