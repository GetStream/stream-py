...
class TrackRecorder(AsyncIOEventEmitter):
    ...
    def __init__(
        self, recorder_id: str, track_type: TrackType, config: RecordingConfig
    ):
        super().__init__()
        ...
        self._file_handle: Optional[Any] = (
            None  # Can be wave.Wave_write or video file handle
        )
    ...
    async def stop_recording(self):
        ...
        try:
            ...
            logger.info(
                f"Stopped {self.track_type.value} recording {self.recorder_id} after {duration:.2f}s ({self._frame_count} frames)"
            )
            ...
            try:
                ...
                logger.error(
                    f"Error during cleanup for {self.recorder_id}: {cleanup_error}"
                )
    ...
    async def queue_frame(self, frame: FrameType):
        ...
        if self.track_type == TrackType.AUDIO and not isinstance(frame, AudioFrame):
            logger.warning(
                f"Received non-audio frame for audio recorder {self.recorder_id}"
            )
            return
        if self.track_type == TrackType.VIDEO and not isinstance(frame, VideoFrame):
            logger.warning(
                f"Received non-video frame for video recorder {self.recorder_id}"
            )
            return
        ...
        while self._running:
            try:
                frame = await asyncio.wait_for(
                    self._frame_queue.get(), timeout=self.FRAME_WORKER_TIMEOUT
                )
    ...
    async def _cleanup_file(self):
        ...
        if self._file_handle:
            ...
            try:
                ...
                await asyncio.get_event_loop().run_in_executor(None, file_handle.close)
            except Exception as e:
...
class AudioTrackRecorder(TrackRecorder):
    ...
    async def _process_frame(self, frame: FrameType) -> Optional[bytes]:
        ...
        if audio_frame.user_id != self.user_id:
            logger.warning(
                f"Frame user_id {audio_frame.user_id} doesn't match recorder user_id {self.user_id}"
            )
            return None
...
class CompositeAudioRecorder(TrackRecorder):
    ...
    def __init__(
        self, config: RecordingConfig, target_user_ids: Optional[Set[str]] = None
    ):
    ...
    async def _process_frame(self, frame: FrameType) -> Optional[bytes]:
        ...
        user_absolute_time = (
            self._user_start_times[audio_frame.user_id] + relative_timestamp
        )
...
class RecordingManager(AsyncIOEventEmitter):
    ...
    def record_audio_data(self, pcm_data, user_id: str):
        ...
        try:
            ...
            if (
                ...
                and self._composite_audio_recorder
            ):
    ...
    async def on_track_removed(self, user_id: str, track_type: str):
        ...
        if track_type == "audio" and user_id in self._audio_recorders:
            await self._stop_user_recording(user_id)
            logger.info(
                f"Stopped recording for user {user_id} whose audio track was removed"
            )
    ...
    async def _start_recording_type(self, recording_type: RecordingType):
        ...
        if recording_type == RecordingType.COMPOSITE:
            self._composite_audio_recorder = CompositeAudioRecorder(
                self.config, self._target_user_ids
            )
