...
class PeerConnectionManager:
    ...
    async def setup_subscriber(self):
        ...
        if not self.subscriber_pc or self.subscriber_pc.connectionState in ["closed", "failed"]:
            self.subscriber_pc = SubscriberPeerConnection(connection=self.connection_manager)
            ...
            async def on_track_added(track, user):
                ...
                await self.connection_manager.recording_manager.on_track_received(track, user)
    ...
    async def add_tracks(
        ...
        async with self.publisher_negotiation_lock:
            ...
            if self.publisher_pc is None:
                self.publisher_pc = PublisherPeerConnection(manager=self.connection_manager)
            ...
            try:
                response = await self.connection_manager.twirp_signaling_client.SetPublisher(
    ...
    async def restore_published_tracks(self):
        ...
        track_ids = list(self.connection_manager.reconnector.reconnection_info.published_tracks.keys())
        ...
        for track_id, track_info in self.connection_manager.reconnector.reconnection_info.published_tracks.items():
