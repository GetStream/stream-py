...
class _ReconnectionInfo:
    ...
    def add_published_track(
        self, track_id: str, track: Any, track_info: Any, media_relay: MediaRelay = None
    ):
...
class ConnectionManager(AsyncIOEventEmitter):
    ...
    def connection_state(self, value: ConnectionState):
        ...
        if value != self._connection_state:
            logger.debug(
                f"Connection state changed from {self._connection_state.value} to {value.value}"
            )
    ...
    async def _join_call_coordinator(
        ...
        **kwargs,
        ...
        try:
            ...
            if local_sfu:
                join_response.data.credentials.server.url = (
                    "http://127.0.0.1:3031/twirp"
                )

            logger.debug(
                f"Received SFU credentials: {join_response.data.credentials.server.url}"
            )
        ...
        try:
            ...
            if connection_options:
                ...
                if connection_options.migrating_from:
                    join_request.reconnect_details.migrating_from = (
                        connection_options.migrating_from
                    )
                if connection_options.previous_session_id:
                    join_request.reconnect_details.previous_session_id = (
                        connection_options.previous_session_id
                    )
    ...
    async def _connect_internal(
        ...
        if (signaling_client is None) != (signaling_context is None):
            raise ValueError(
                "signaling_client and signaling_context must be provided together or both be None"
            )
        ...
        if not ws_client:
            try:
                self.ws_client, sfu_event = await self._connect_websocket(
                    token, ws_url, current_session_id, self._connection_options
                )
        ...
        if not signaling_client or not signaling_context:
            ...
            self.twirp_signaling_client, self.twirp_context = (
                self._setup_signaling_client(token, server_url)
            )
        else:
    ...
    @retry(
        ...
        wait=wait_random_exponential(
            multiplier=_DEFAULT_MIN_WAIT, max=_DEFAULT_MAX_WAIT
        ),
        retry=is_retryable,
    ...
    def _register_websocket_event_handlers(self):
        ...
        async def handle_error_event(event):
            ...
            self.emit(
                "sfu_error",
                {
                    "reconnect_strategy": getattr(
                        event, "reconnect_strategy", _ReconnectionStrategy.UNSPECIFIED
                    ),
    ...
    async def add_tracks(
        ...
        if original_audio:
            ...
            self._reconnection_info.add_published_track(
                original_audio.id,
                original_audio,
    ...
    async def _execute_reconnection_loop(self, reconnect_start_time: float):
        ...
        while True:
            ...
            try:
                ...
                logger.info(
                    f"Executing reconnection with strategy {self._reconnection_info.strategy}"
                )
        ...
        try:
            if self.ws_client and self.ws_client.running:
                ...
                await self._connect_internal(
                    ...
                    token=self.join_response.data.credentials.token
    ...
    async def _reconnect_migrate(self):
        ...
        try:
            ...
            if hasattr(self, "_connection_options"):
                ...
                self._connection_options.previous_session_id = (
                    self.session_id if migrating_from else None
                )

            # Use _connect_internal for migration
            await self._connect_internal(
                region=self._connection_options.region, session_id=self.session_id
            )
        ...
        async def handle_network_change(event_data):
            ...
            if not online:
                ...
                if self._reconnection_info.last_offline_timestamp:
                    offline_duration = (
                        time.time() - self._reconnection_info.last_offline_timestamp
                    )
                    strategy = (
                        _ReconnectionStrategy.FAST
                ...
                asyncio.create_task(self._reconnect(strategy, "Going online"))

        @self.on("go_away")
        async def handle_go_away(event_data):
            asyncio.create_task(
                self._reconnect(
                    _ReconnectionStrategy.MIGRATE, "Server requested migration"
                )
            )

        @self.on("sfu_error")
        async def handle_sfu_error(event_data):
            strategy = event_data.get(
                "reconnect_strategy", _ReconnectionStrategy.UNSPECIFIED
            )
