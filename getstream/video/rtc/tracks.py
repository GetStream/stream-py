...
from getstream.video.rtc.pb.stream.video.sfu.models.models_pb2 import TrackInfo, TrackType
...
class TrackSubscriptionConfig:
    ...
    video_dimension: VideoDimension = field(default_factory=lambda: VideoDimension(width=1280, height=720))
    screenshare_dimension: VideoDimension = field(default_factory=lambda: VideoDimension(width=1920, height=1080))
...
class SubscriptionConfig:
    ...
    default: TrackSubscriptionConfig = field(default_factory=lambda: TrackSubscriptionConfig())
...
class TrackPublisher:
    ...
    def __init__(self, manager: "Any") -> None:  # forward ref to avoid circular import typings
    ...
    async def add_tracks(
        ...
        async with m.publisher_negotiation_lock:
            ...
            if m.publisher_pc is None:
                from getstream.video.rtc.pc import PublisherPeerConnection  # local import to avoid cycle
            ...
            try:
                response = await m.twirp_signaling_client.SetPublisher(
                    ...
                    request=signal_pb2.SetPublisherRequest(
                        session_id=m.session_id, sdp=m.publisher_pc.localDescription.sdp, tracks=track_infos
                    ),
                    server_path_prefix="",
                )
        ...
        if original_audio:
            m._reconnection_info.add_published_track(
                original_audio.id, original_audio, track_infos[track_info_index], audio_relay
            )
            track_info_index += 1
        if original_video:
            m._reconnection_info.add_published_track(
                original_video.id, original_video, track_infos[track_info_index], video_relay
            )

    async def add_track(
        self, track: aiortc.mediastreams.MediaStreamTrack, track_info: Optional[TrackInfo] = None
    ):
...
class SubscriptionManager(AsyncIOEventEmitter):
    ...
    def __init__(self, connection_manager: "Any", subscription_config: Optional[SubscriptionConfig] = None) -> None:
        super().__init__()
    ...
    def _get_role_config(self, participant: Optional[models_pb2.Participant]) -> Optional[TrackSubscriptionConfig]:
    ...
    def _should_subscribe(self, participant: Optional[models_pb2.Participant], track_type: int) -> bool:
    ...
    def _create_track_subscription_detail(
        ...
        try:
            ...
            elif track_type == TrackType.TRACK_TYPE_SCREEN_SHARE and cfg.screenshare_dimension:
    ...
    def _generate_subscription_details_for_participants(self) -> List[signal_pb2.TrackSubscriptionDetails]:
        ...
        for participant in self.connection_manager.participants_state._participant_by_prefix.values():
            ...
            for track_type in participant.published_tracks:
                ...
                if (
                    self._subscription_config.max_subscriptions is not None and
                    len(subscription_infos) >= self._subscription_config.max_subscriptions
                ):
    ...
    async def handle_track_published(self, event):
        ...
        try:
            ...
            if hasattr(event, 'participant'):
                self.connection_manager.participants_state.add_participant(event.participant)

            track_type = getattr(event, 'type', None)
            if track_type is None:
                return

            if not self._should_subscribe(getattr(event, 'participant', None), track_type):
                return

            # Check for duplicates & subscription limits
            async with self._lock:
                already_subscribed = any(
                    d.user_id == event.user_id and d.session_id == event.session_id and d.track_type == track_type
                ...
                if (
                    self._subscription_config.max_subscriptions is not None and
                    len(self._subscribed_track_details) >= self._subscription_config.max_subscriptions
                ):
                    logger.info("Max subscription limit reached, skipping new track subscription")
                    return
        ...
        try:
            ...
            async with self._lock:
                ...
                self._subscribed_track_details = [
                    d for d in self._subscribed_track_details
                    if not (d.user_id == user_id and d.session_id == session_id and d.track_type == track_type)
                ]
