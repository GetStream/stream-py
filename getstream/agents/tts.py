...
class TTS(AsyncIOEventEmitter, abc.ABC):
    ...
    @abc.abstractmethod
    async def synthesize(self, text: str, *args, **kwargs) -> Union[bytes, Iterator[bytes], AsyncIterator[bytes]]:
        """
    ...
    async def send(self, text: str, user: Optional[Dict[str, Any]] = None, *args, **kwargs):
        """
        ...
        elif hasattr(audio_data, '__iter__') and not isinstance(audio_data, (str, bytes, bytearray)):
            for chunk in audio_data:
                await self._track.write(chunk)
                self.emit("audio", chunk, user)
        else:
            raise TypeError(f"Unsupported return type from synthesize: {type(audio_data)}")
