...
class Feed:
    def __init__(
        self, client, feed_type: str, feed_id: str = None, custom_data: Optional[Dict] = None
    ):
        self.id = feed_id
    ...
    def _sync_from_response(self, data):
        if hasattr(data, "feed") and hasattr(data.feed, "custom"):
            self.custom_data = data.feed.custom

    def get_feed_identifier(self) -> str:
        """
        Get the full feed identifier (feed_type:feed_id)
        """
        return f"{self.feed_type}:{self.id}"
    ...
    def connect_openai(
        ...
        return ConnectionManagerWrapper(connection_manager, self.feed_type, self.id)

    def delete(
        self, hard_delete: Optional[bool] = None
    ) -> StreamResponse[DeleteFeedResponse]:
        response = self.client.delete_feed(
            feed_group_id=self.feed_type, feed_id=self.id, hard_delete=hard_delete
        )
    ...
    def update(
        ...
        response = self.client.update_feed(
            feed_group_id=self.feed_type, feed_id=self.id, created_by_id=created_by_id, custom=custom
        )
        self._sync_from_response(response.data)
        return response
    ...
    def unpin_activity(
        ...
        response = self.client.unpin_activity(
            feed_group_id=self.feed_type, feed_id=self.id, activity_id=activity_id, user_id=user_id
        )
        self._sync_from_response(response.data)
        return response
    ...
    def accept_feed_member_invite(
        ...
        response = self.client.accept_feed_member_invite(
            feed_group_id=self.feed_type, feed_id=self.id, user_id=user_id, user=user
        )
        self._sync_from_response(response.data)
        return response
