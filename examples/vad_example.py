...
async def main():
    ...
    if not api_key or not api_secret:
        logger.error("Please set STREAM_API_KEY and STREAM_API_SECRET environment variables")
        return
    ...
    async def on_speech_detected(pcm_data, user):
        ...
        if user:
            logger.info(f"Speech detected from {user.get('user_id', 'unknown')}: {duration:.2f} seconds")
        else:
