...
class TestTrackRecorder:
    ...
    def test_video_track_not_implemented(self):
        ...
        with pytest.raises(NotImplementedError, match="Video recording is not yet implemented"):
    ...
    async def test_multi_user_mixing(self, recorder):
        ...
        for i in range(10):
            for user_id in users:
                audio_data = np.random.randint(-16000, 16000, 960, dtype=np.int16).tobytes()
                frame = AudioFrame(
                    user_id=user_id,
                    data=audio_data,
                    timestamp=time.time(),
...
class TestFileOperations:
    ...
    async def test_concurrent_recorders(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            recorders = [
                AudioTrackRecorder(f"user_{i}", config)
                for i in range(3)
            ]
